...
export function decode(code1, code2) {
  ...
  for (let i = 0; i < newCode2.length; i += 2) {
    ...
    if (position[letter] !== undefined) {
      indices.push(position[letter] + number);
    }
  }
...
export function encode(pwd) {
  ...
  for (let i = 0; i < pwd.length; i++) {
    const randomIndex = Math.floor(Math.random() * code1Array.length);
    pwdRandom.push(code1Array[randomIndex]);
  }

  for (let i = 0; i < pwd.length; i++) {
    if (pwdRandom[i] < code1Array.length) {
      code1Array[pwdRandom[i]] = pwd[i];
    }
  }
